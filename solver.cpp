...
int SATSolverDPLL::DPLL(Formula f) {
  ...
  if (result == Cat::satisfied) return Cat::satisfied;
  if (result == Cat::unsatisfied) return Cat::unsatisfied;

  // Encontrando a variavel que mais apareceu
  int max_freq = -1;
  ...
  for (int j = 0; j < 2; j++) {
    ...
    if (transform_result == Cat::satisfied) return Cat::satisfied;
    if (transform_result == Cat::unsatisfied) continue;

    if (DPLL(new_f) == Cat::satisfied) return Cat::satisfied;
  }
...
bool check_difference(Formula F_base, vector<int> C_alvo, SATSolverDPLL &solver) {
  for (int i = 0; i < C_alvo.size(); i++) {
    ...
    //Cria um vetor temporário só pra guardar esse literal sozinho
...
bool check_equivalence(Formula f1, Formula f2, SATSolverDPLL &solver) {
  // Ajusta a quantidade de variaveis pegando a maior
